...
async def _cleanup_expired_task():
    ...
    while True:
        try:
            ...
            session.query(IdempotencyRecord).filter(IdempotencyRecord.expires_at != None).filter(
                IdempotencyRecord.expires_at < now).delete(synchronize_session=False)
            session.commit()
        except Exception:
            pass
        finally:
    ...
    try:
        ...
        rec = IdempotencyRecord(
            ...
            expires_at=(datetime.now(timezone.utc) +
                        timedelta(days=IDEMPOTENCY_TTL_DAYS)),
        )
        session.add(rec)
        session.commit()
